...
public class GoTo extends DialogFragment
{
	...
	{
		final Dialog d = new Dialog(mainActivity, getTheme());
		d.requestWindowFeature(Window.FEATURE_NO_TITLE);
		d.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_VISIBLE | WindowManager.LayoutParams.SOFT_INPUT_ADJUST_NOTHING);
		return d;
	}
	...
	public void onViewCreated(@NonNull View view, Bundle savedInstanceState)
	{
		...
		if(mArgs != null)
			index = mArgs.getInt("index", 0);

		final PagerAdapter pageAdapter = new PagerAdapter(getChildFragmentManager(), getLifecycle(), index);
		viewPager.setAdapter(pageAdapter);
		viewPager.setOffscreenPageLimit(pageAdapter.getItemCount());
		viewPager.setCurrentItem(index, false); // will not work for number, since getChildFragmentManager().getFragments() is empty. even if put this code in onActivityCreated(Bundle savedInstanceState). solution is in MyEditText.onWindowFocusChanged
		...
		tabLayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener()
		{
			...
			{
				final List<Fragment> f = getChildFragmentManager().getFragments(); // after creation this is empty ! so it will not conflict with MyEditText.onWindowFocusChanged
				for (Fragment fr : f)
				{
					if (fr instanceof PageNumberFragment)
					{
						...
						{
							if (mainActivity.getCurrentFocus() != null) // If no view is focused, an NPE will be thrown with getWindowToken() e.g. when changing the orientation
								imm.hideSoftInputFromWindow(pf.pageText.getWindowToken(), 0);
						}
						break;
					}
				}
			...
			if (window != null)
			{
				...
				if (getActivity() != null)
				{
					...
					if (windowManager != null)
					{
						...
						//params.height = (int) (getActivity().getWindow().getDecorView().getHeight()*0.9); // https://stackoverflow.com/questions/55803448/how-to-prevent-dialogfragment-from-resize
	...
	{
		final int index;
		PagerAdapter(FragmentManager fm, @NonNull Lifecycle lc, int index)
		{
			super(fm, lc);
			this.index = index;
		}
		...
		public Fragment createFragment(int position)
		{
			switch (position)
			{
				...
				default:
					return PageNumberFragment.newInstance(index);
			}
		}
