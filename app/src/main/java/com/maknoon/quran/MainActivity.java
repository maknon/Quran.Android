...
public class MainActivity extends AppCompatActivity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		if (savedInstanceState != null) // when recreate() this activity if language is change. savedInstanceState will be saved so we need to force the direction
		{
			...
			final int colorMode = getResources().getConfiguration().uiMode & Configuration.UI_MODE_NIGHT_MASK;
			nightMode = colorMode == Configuration.UI_MODE_NIGHT_YES;
		...
		darkMode = menu.findItem(R.id.darkmode);
		...
		if (nightMode)
		{
			darkMode.setTitle(R.string.lightmode);
			darkMode.setIcon(R.drawable.baseline_light_mode_24);
		}
		else
		{
			darkMode.setTitle(R.string.darkmode);
			darkMode.setIcon(R.drawable.baseline_dark_mode_24);
		}

		ViewCompat.setOnApplyWindowInsetsListener(toolbar, (v, insets) -> {
			final Insets ins = insets.getInsets(WindowInsetsCompat.Type.systemGestures()); // systemBars() working as well
			v.setPadding(ins.left, ins.top, ins.right, 0);
			//return WindowInsetsCompat.CONSUMED;
			return insets;
		});

		ViewCompat.setOnApplyWindowInsetsListener(mViewPager, (v, insets) -> {
			final Insets ins = insets.getInsets(WindowInsetsCompat.Type.systemGestures());
		...
		/* Not needed any more
		https://stackoverflow.com/questions/59195449/viewpager2-onapplywindowinsetslistener-not-called

		if(mViewPager.isAttachedToWindow())
	...
	public void onConfigurationChanged(@NonNull Configuration newConfig)
	{
		...
		final int nightModeFlags = newConfig.uiMode & Configuration.UI_MODE_NIGHT_MASK;
		switch (nightModeFlags) {
			case Configuration.UI_MODE_NIGHT_YES:
				nightMode = true;
				//refresh(); // not reflecting the color changes in toolbar in darkmode
				recreate();
				break;
